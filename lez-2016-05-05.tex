...
\begin{teo}[Altro teorema di trasversalità $\pitchfork$]
...
\begin{equation*}
\chi(W) = \sum_{x_j~p.to~critico} (-1)^{\lambda _j} = \sum_{x_j~p.to~critico} (-1)^{n - \lambda _j} 
\end{equation*}
...
\begin{prop}
Se $\text{dim}W = n$ è dispari $\implies$ $\chi(W) = -\chi(W) = 0$
\end{prop}
