...
$$ \Fundef{X}{\Fundef[\pi]{U \times V}{V}}$$ \marginpar{fig1}
...
$$D(X) := \quoset{X \sqcup X}{(\fundef[id]{\boundary X}{\boundary X})}$$ \marginpar{fig2}
...
Sia $\pitchfork_K(X,Y,A) := \setdef[\fundef{X}{Y}]{f \pitchfork A \text{ lungo } K}$. Dimostriamo che $\forall K$ $\pitchfork_K(X,Y,A)$ è (un aperto) denso. \marginpar{fig3} \\
