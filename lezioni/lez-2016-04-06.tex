...
%chi ti ha detto che esista un atlante con la proprietà 2?
%proposta: prendo un atlante, faccio in modo che ogni punto caschi in un C_i, prendendo un'altra carta per ogni punto, simile ad una che ho già in cui compare il punto in questione, con l'immagine opportunamente traslata, cosicché il punto caschi in 0, e la aggiungo all'atlante; essendo traslazioni i cambiamenti di carta sono ovviamente lisci, quindi continua ad essere un atlante, e i C_i ricoprono X, a questo punto poiché X è compatto estraggo su i C_i: una sottofamiglia di carte è ovviamente compatibile, perchè lo era nell'atlante di partenza, e poiché i C_i ricoprono lo fanno anche le carte in cui sono contenuti, quindi è a tutti gli effetti un atlante e soddisfa la proprietà 2. La proprietà 1 è facilmente soddisfatta, prendendo in partenza per l'altro un atlante finito, magari ottenuto estraendo da uno a caso, e scegliendo nell'immagine di ogni carta una palla centrata in 0 e intersecandole tutte: ottengo un'altra palla aperta che soddisfa la proprietà 1 e la chiamo B_2 (se voglio ovviamente, riscalando opportunamente le carte, non c'è problema a fare in modo che abbia proprio raggio 2), e poi riparto con il procedimento di sopra
 \begin{itemize}
...
\end{figure}
\marginpar{la figura fa schifo: manca lo 0, inutile ma citato, i nomi sono sparsi a casaccio}
...
%dovrebbe esserlo ovunque! in particolare dovrebbe esserlo in phi^{-1}(B_2(0)), per il resto è nulla
...
%in particolare \lambda_i manda aperti, i C_i, in chiuso, il singoletto {1}, quindi non so come faccia g a mandare aperti in aperti
  \qedhere
 \end {itemize}
\end{proof}
