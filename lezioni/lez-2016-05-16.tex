...
Sia $X$ una superficie compatta, chiusa, connessa. $V = \eta _1(X)$, il prodotto scalare non degenere è la forma di intersezione precedentemente definita. Dato un diffeomorfismo tra due varietà:
...
Per ogni classe di isometria degli spazi vettoriali in caratteristica due esiste una varietà $X$ che rappresenta la classe di isometria data tramite lo spazio $(\eta(S), \beta)$.
...
Come si vede nella figura costruisco un $S^1$ identificando i punti antipodali ($\quoset{S^1}\sim$), si vede che il bordo della calotta ricopre due volte $\quoset{S^1}\sim$. Il collare che ha per bordo questi due anelli è chiamato nastro di Möbius e si indica con $\mathcal{M}$. 
...
\marginpar{Ogni laccio sul nastro di Möbius lo taglia a metà. Da ricordare che il nastro di Möbius ha "una faccia sola", cioè, essendo un oggetto bidimensionale, non distingue un punto sopra da un punto sotto: se è sul nastro il punto è uno solo. Quindi quando perturbo poco un laccio ogni punto può scegliere di andare a destra e a sinistra, ma non posso fare il doppio giro. Quindi, poiché a un certo punto la destra e la sinistra si "scambiano", per mantenere la continuità il laccio perturbato deve intersecare quello imperturbato (Ale)}
