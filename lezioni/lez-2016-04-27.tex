...
Sia $i$ una immersione cioè $X \hookrightarrow A \subset Y$ e definisco $i^{-1} (A) = X \pitchfork A$. Si ha anche $T_{x}X + T_{x}A = T_{x}Y$ e definisco $T_{x}(X \cap A) = T_{x}X \cap T_{y}A$. 
