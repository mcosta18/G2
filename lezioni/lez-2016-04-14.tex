...
\footnotetext{Cioè la dimensione dello spazio vettoriale è pari al numero di componenti connesse della varietà obiettivo}
...
\marginpar{FIGURA di curve trasverse, punti su cui verificare - C'è, basta guardare, attualmente è la numero 21 nel pdf (Ale)}
\end{es}
...
\marginpar{FIGURA? - inutile, verrebbe identica a quella del caso senza bordo, al massimo si può esplicitare il rimando all'altra (Ale)}
\end{defn}
...
\marginpar{Questa frase è torbida - obiezione respinta in mancanza di ulteriori argomentazioni (Ale)}
\end{itemize}
