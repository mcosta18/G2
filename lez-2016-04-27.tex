...
\titletit{Fine della lezione sulla trasversalità}
...
%Aggiungere disegno incomprensibile 1
...
%Aggiungere disegno incoprensibile 2

\begin{def}[Trasversalità di funzioni]
...
\begin{def}[Versione orientata]

\end{def}

\begin{def}[Versione non orientata]
...
Dunque vediamo che il grado di una funzione tra varietà non orientate è una funzione $ \mathcal{E}(X, Y) \rightarrow \mathbb{Z} / 2 mathbb{Z}$.
...
\begin{dim}
 Dimostreremo che a $y_{0}$ fissato il grado non dipende dalla scelta della $g$. Siano $g_{1}$ e $g_{2}$ tali che $g_1 \pitchfork y_0$ e $g_{2} \pitchfork y_{0}$ allora la $g1$ e la $g2$ 
 sono tra loro omotope.
 
 %Inserisco il disegnino cancer di g1 e g2 che sono cobordanti e mostro eplicitamente dove stanno gli insiemi di cui sto parlando
...
$\fundef[H]{Y \times [0,1]}{Y}$ tale che posto $H_{t} = H_{Y \times {t}}$ si ha che $H_{0} = Id$. $H_{t}$ è un diffeomorfismo di Y $\forallt \in [0,1]$  e vale $H_{0} = x$ e $H_{1} = y$.
